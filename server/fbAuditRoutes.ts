...
export function setupFbAuditRoutes(app: Express) {
  ...
  app.get('/api/fbaudit/debug-purchase/:accountId', requireJWTAuth, async (req: Request, res: Response) => {
    try {
      ...
      return res.json({
        ...
        debugInfo: {
          accountId: accountId,
          dataSource: 'Facebook Marketing API v24.0 with enhanced aggregation',
  ...
  app.get('/api/fbaudit/test-api/:accountId', requireJWTAuth, async (req: Request, res: Response) => {
    try {
      ...
      const url = `https://graph.facebook.com/v24.0/${formattedAccountId}/insights?fields=${testFields.join(',')}&time_range={"since":"${since}","until":"${until}"}&access_token=${user.metaAccessToken}`;
